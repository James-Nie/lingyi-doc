...
import type { BaseViewType, BaseSheetModel, ColumnDef, FreeTable, RecordRow } from '@lingyi-doc/core';
import { isGroupableColumn } from '@lingyi-doc/core';
import {
...
export function useBaseColumnMenu({
  ...
  }, [columnDefs, table, dirtyTrackerRef, scheduleRender]);
  ...
  const handleFilterByField = useCallback((fieldId: string) => {
    ...
    if (!already) {
      ...
      const operator = (type === 'number' || type === 'currency' || type === 'percent' || type === 'date' || type === 'datetime')
        ? 'eq' as const
