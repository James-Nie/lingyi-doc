...
import {
  ...
  buildDisplayRowHeights,
  applyFilterToDisplayRowHeights,
  getCellText,
  recordRowHasFieldData,
  resolveGroupContextFromRecord,
  createFilterConditionFromCell,
...
export interface UseBaseGridStateOptions extends Pick<
  SheetContainerProps,
  'table' | 'onOpenFieldConfig' | 'onToggleFieldVisibility' | 'onDeleteField' | 'onAddSheetComment' | 'commentsEnabled'
> {
...
export function useBaseGridState({
  ...
  const displayRowHeights = useMemo(() => {
    ...
    if (hasActiveViewFilter) {
      return applyFilterToDisplayRowHeights(
        sheet.rowCount,
        sheetRows,
        sheet.rowHeights,
        collapsedRowIdSet,
  ...
  const handleBaseAddComment = useCallback((rowIndex: number, colIndex: number) => {
    ...
    const colDef = colIndex >= 0 ? columnDefs[colIndex] : undefined;
    const cell = colIndex >= 0 ? table.getCell(rowIndex, colIndex) : null;
    const cellText = cell ? getCellText(cell.value).trim() : '';
    const fieldName = colDef?.name ?? '';
    const quote = cellText
      ? `${fieldName}: ${cellText}`.slice(0, 200)
      : `记录 ${rowIndex + 1}`;
  ...
  }, [commentsEnabled, onAddSheetComment, sheetRows, columnDefs, table]);
