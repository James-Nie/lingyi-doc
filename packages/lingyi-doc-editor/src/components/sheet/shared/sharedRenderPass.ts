import {
  BASE_THEME,
  clipCanvasToScrollablePane,
  drawFillHandle,
  getFillHandleAnchor,
  getFilteredColumnIndices,
  normalizeRange,
  RENDER_LAYERS,
  resolveCellBackgroundFillColor,
...
export function drawCommentHighlightLayer(ctx: SheetRenderPassContext): void {
  const { layerManager, renderer, sheet, mergeRanges, containerSize, sheetCommentCells, selectedCommentId } = ctx;
  if (!sheetCommentCells?.length) return;

  const bgCtx = layerManager.getLayer(RENDER_LAYERS.BACKGROUND);
  for (const cell of sheetCommentCells) {
    const selected = selectedCommentId === cell.threadId;
...
export function drawCommentMarkerLayer(
  ...
  const { layerManager, renderer, sheet, mergeRanges, containerSize, sheetCommentCells } = ctx;
  if (!sheetCommentCells?.length) return;
...
export function drawOverlayLayer(
  ...
  if (frozenCols > 0 || frozenRows > 0) {
    ...
    if (frozenRows > 0) {
      const frozenBoundaryY = viewport.getRowScreenTop(frozenRows, ctx.activeRowHeights);
      overlayCtx.beginPath();
