...
import { getBuiltinShapeKinds, getShapeRegistry } from '@lingyi-doc/core';
import {
  appendShapePath,
  drawShapeBodyImpl,
...
/** 通过 registry 调用图形能力 */
export function invokeShapeCapability(
  kind: ShapeKind,
  method: keyof import('@lingyi-doc/core').ShapeCapabilities,
  ...args: unknown[]
): unknown {
  return getShapeRegistry().invoke(kind, method, ...(args as never[]));
}
