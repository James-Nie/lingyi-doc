...
export interface NativeTextSelectionDetail {
  ...
  collapsed: boolean;
  /** 选区方向：true 表示从后向前选中（anchor 在 focus 之后） */
  backward?: boolean;
}
...
export function getNativeTextSelectionDetail(
  ...
  // 检测选区方向：DOM anchor 在 focus 之后即为反向选中
  const backward = sel.anchorNode && sel.focusNode
  ...
  if (lo === hi) {
    ...
    if (block?.type === 'list' && root) {
      ...
      if (ctx && !ctx.collapsed) {
        ...
        return { slices: mergeTextSelectionSlices(slices), collapsed: false, backward };
      }
    }
    pushTextBlockSlices(slices, blocks, blockEls, range, startBlockIdx, endBlockIdx);
    const merged = mergeTextSelectionSlices(slices);
    return { slices: merged, collapsed: merged.length === 0, backward };
  }
...
export function selectionSlicesToAnchors(
  ...
  // 文档顺序的起始端和末尾端
  const docStart: DocAnchor = first.listItemIndex != null
    ? blockAnchor(first.blockIndex, {
        kind: 'list',
        itemIndex: first.listItemIndex,
        offset: first.start,
      })
    : blockAnchor(first.blockIndex, { kind: 'text', offset: first.start });

  const docEnd: DocAnchor = last.listItemIndex != null
    ? blockAnchor(last.blockIndex, {
        kind: 'list',
        itemIndex: last.listItemIndex ?? first.listItemIndex,
        offset: last.end,
      })
    : blockAnchor(last.blockIndex, { kind: 'text', offset: last.end });

  // backward 时，DOM anchor 在文档末尾位置（用户开始拖拽的位置），focus 在文档起始位置
  // 所以我们的 anchor 应映射到 docEnd，focus 映射到 docStart
  if (backward) {
    return { anchor: docEnd, focus: docStart };
  }
  return { anchor: docStart, focus: docEnd };
}
