...
const DOC_FIND_ACTIVE_BG = '#F5B075';
const DOC_FIND_MATCH_BG = '#FFF066';
...
function renderTabSpan(
  textBeforeOnLine: string,
  textAfter: string,
  options?: MarksToHtmlOptions,
): string {
  const stops = resolveTabStops(options?.tabStops);
  const fontSizePx = options?.fontSizePx ?? 15;
  const { width, align, leader } = computeTabSpanStyle(textBeforeOnLine, textAfter, stops, fontSizePx);
  const leaderStyle = leaderCss(leader);
  const barStyle = align === 'bar' ? 'border-left:1px solid #1F2329;box-sizing:border-box;' : '';
  return `<span data-doc-tab="${align}" contenteditable="false" style="display:inline-block;width:${width}px;min-width:${width}px;height:1em;vertical-align:baseline;${leaderStyle}${barStyle}">\u200B</span>`;
}
...
function escapeHtmlPreservingTabs(s: string, linePrefix: string, options?: MarksToHtmlOptions): string {
  if (!s.includes('\t')) return escapeHtml(s);
  const parts = s.split('\t');
  let before = linePrefix;
  let html = '';
  for (let i = 0; i < parts.length; i++) {
    ...
    if (i < parts.length - 1) {
      ...
      html += renderTabSpan(before, after, options);
      before += '\t';
    }
  }
  return html;
...
function renderMarkedLine(text: string, marks: TextMark[], options?: MarksToHtmlOptions): string {
  if (!text) return '';
  const findHighlights = (options?.findHighlights ?? []).filter(
    h => h.start < h.end && h.start >= 0 && h.end <= text.length,
  );
  if (!marks.length && !findHighlights.length) return escapeHtmlPreservingTabs(text, '', options);
  ...
  // 制表符边界，避免 mark 跨 tab 时把 \t 包进格式标签导致提取困难
  ...
  for (let i = 0; i < points.length - 1; i++) {
    ...
    if (segment === '\t') {
      html += renderTabSpan(text.slice(0, start), text.slice(end), options);
      continue;
    }
    const escaped = escapeHtmlPreservingTabs(segment, text.slice(0, start), options);
    const active = sorted.filter(m => m.start <= start && m.end >= end);
    html += wrapSegment(escaped, active, options, start, end);
  }
  return html || escapeHtmlPreservingTabs(text, '', options);
}
...
function wrapSegment(
  ...
  const findHit = options?.findHighlights?.find(h => h.start <= rangeStart && h.end >= rangeEnd && rangeStart < rangeEnd);
  if (findHit) {
    const bg = findHit.active ? DOC_FIND_ACTIVE_BG : DOC_FIND_MATCH_BG;
    result = `<span data-doc-find="${findHit.active ? 'active' : 'match'}" style="background:${bg}">${result}</span>`;
  }
  return result;
}
...
export function setCaretOffset(el: HTMLElement, offset: number): void {
  ...
  const walk = (node: Node): boolean => {
    if (node.nodeType === Node.TEXT_NODE) {
      ...
      if (remaining <= len) {
        // map into original textContent accounting for stripped ZWSP roughly by using raw index
        const range = document.createRange();
    ...
    if (node.nodeType === Node.ELEMENT_NODE) {
      ...
      if (elem.dataset.docTab !== undefined) {
        if (remaining <= 0) {
          const range = document.createRange();
          range.setStartBefore(node);
          range.collapse(true);
...
/** 图/表/代码等对象块（方向键可整块选中） */
export function isObjectBlock(block: DocBlock): boolean {
  return (
    block.type === 'image'
    || block.type === 'table'
    || block.type === 'code'
...
export const INDENT_STEP_PX = DEFAULT_TAB_STOP_PX;
/** 首行缩进约两字符（与默认制表位一致） */
export const FIRST_LINE_INDENT = '2ch';
...
function marksFromElement(el: HTMLElement): ActiveMark[] {
  ...
  while (node) {
    ...
    // 查找高亮为临时 DOM，不写入 background mark
    if (node.dataset.docFind === undefined) {
