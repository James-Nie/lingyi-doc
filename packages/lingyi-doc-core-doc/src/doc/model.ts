...
import type { PendingCaretSpec } from './caret';

type HistoryEntry = {
  blocks: DocBlock[];
  caret: PendingCaretSpec | null;
};
...
export class RichDocument {
  ...
  /** 最近一次 undo/redo 恢复的光标（供编辑器 scheduleCaret） */
  lastRestoredCaret: PendingCaretSpec | null = null;
  ...
  private pushHistory(caret: PendingCaretSpec | null = null): void {
    this.undoStack.push({ blocks: this.snapshot(), caret });
    if (this.undoStack.length > this.maxHistory) this.undoStack.shift();
    this.redoStack = [];
  }

  setBlocks(blocks: DocBlock[], recordHistory = true, caret: PendingCaretSpec | null = null): void {
    if (recordHistory) this.pushHistory(caret);
    this.blocks = blocks;
  }
  ...
  undo(currentCaret: PendingCaretSpec | null = null): boolean {
    if (!this.undoStack.length) return false;
    this.redoStack.push({ blocks: this.snapshot(), caret: currentCaret });
    const entry = this.undoStack.pop()!;
    this.blocks = entry.blocks;
    this.lastRestoredCaret = entry.caret;
    return true;
  }

  redo(currentCaret: PendingCaretSpec | null = null): boolean {
    if (!this.redoStack.length) return false;
    this.undoStack.push({ blocks: this.snapshot(), caret: currentCaret });
