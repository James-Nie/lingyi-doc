import { CellData, CellCoord, CellRange, CellStyle, CellValue, ColumnDef, SelectOption, getCellText, getCellAlign, DEFAULT_CELL_STYLE, DEFAULT_COLUMN_WIDTH, DEFAULT_ROW_HEIGHT, NumberFormat, DateFormat } from '../types/index';
...
export class BaseCellRenderer {
  ...
  drawBaseCellContent(
    ...
    switch (type) {
      ...
      case 'user':         this._drawUserAvatar(ctx, value, columnDef, drawRect); break;
      case 'rating':       this._drawRating(ctx, value, columnDef, drawRect); break;
      case 'progress':     this._drawProgressBar(ctx, value, drawRect); break;
      ...
      case 'currency':     this._drawCurrency(ctx, value, drawRect); break;
      case 'percent':      this._drawPercent(ctx, value, drawRect); break;
      case 'boolean':      this._drawBooleanCheckbox(ctx, value, drawRect); break;
      case 'date':         this._drawDateCell(ctx, value, columnDef, drawRect); break;
      case 'datetime':     this._drawDateCell(ctx, value, columnDef, drawRect); break;
  ...
  private _drawDateCell(
    ...
    const displayText = this._formatDateString(timestamp, formatStr);
  ...
  /** 根据格式字符串格式化日期 */
  private _formatDateString(timestamp: number, formatStr: string): string {
  ...
  private _drawUserAvatar(
    ...
    let userId = '';
    if (value.type === 'text') userId = value.text;
    else if (value.type === 'number') userId = String(value.value);
    ...
    // 尝试获取头像图片
    const avatarUrl = `https://avatar.example.com/${userId}`; // 实际应用中从用户数据获取
    const avatar = this._assetManager.getImage(avatarUrl);

    if (avatar) {
      ctx.drawImage(avatar, cx - radius, cy - radius, avatarSize, avatarSize);
    } else {
      // 默认头像：首字母 + 背景色
      const bgColor = this._getUserColor(userId);
      ctx.fillStyle = bgColor;
      ctx.fillRect(cx - radius, cy - radius, avatarSize, avatarSize);
      ctx.fillStyle = '#fff';
      ctx.font = `bold ${Math.max(10, 12 * zoom)}px Arial, sans-serif`;
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
    ...
    const maxNameWidth = rect.x + rect.width - nameX - 2 * zoom;
    if (maxNameWidth > 10 * zoom) {
      ctx.font = `${Math.max(10, 12 * zoom)}px Arial, sans-serif`;
      ctx.fillStyle = '#333';
      ctx.textAlign = 'left';
      ctx.textBaseline = 'middle';
  ...
  private _drawProgressBar(
    ...
    let progress = 0;
    if (value.type === 'number') progress = Math.max(0, Math.min(100, value.value));
    else if (value.type === 'text') progress = Math.max(0, Math.min(100, parseFloat(value.text) || 0));

    const zoom = this._getZoom();
    const padding = 4 * zoom;
    const barHeight = Math.min(16 * zoom, rect.height - padding * 2);
    ...
    if (fillWidth > 0) {
      ctx.fillStyle = progress >= 100 ? '#4CAF50' : progress >= 60 ? '#2196F3' : '#FF9800';
      this._roundRect(ctx, barX, barY, fillWidth, barHeight, borderRadius);
      ctx.fill();
    }

    // 百分比文字（空值时不显示）
    if (value.type !== 'empty') {
      ctx.font = `bold ${Math.max(9, 10 * zoom)}px Arial, sans-serif`;
      ctx.fillStyle = '#333';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText(`${Math.round(progress)}%`, barX + barWidth / 2, barY + barHeight / 2);
    }
  }
  ...
  private _drawCurrency(
    ...
    const zoom = this._getZoom();
    const padding = this._getPadding();
    const symbol = (value.type === 'number' && value.format?.kind === 'currency') ? value.format.symbol : '¥';
    const decimals = (value.type === 'number' && value.format?.kind === 'currency') ? value.format.decimals : 2;

    const parts = Math.abs(num).toFixed(Math.max(0, Math.min(10, decimals))).split('.');
    parts[0] = parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, ',');
    const text = `${num < 0 ? '-' : ''}${symbol}${parts.join('.')}`;
