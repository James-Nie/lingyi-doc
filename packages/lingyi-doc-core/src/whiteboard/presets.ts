...
import { createWhiteboardMeasureOptions, WHITEBOARD_MIND_BRANCH_STYLE_DEFAULT } from '../mindnote/utils';

const MINDMAP_EMBED_PADDING = 16;
const MINDMAP_MIN_W = 160;
const MINDMAP_MIN_H = 120;

function mindmapElementSize(root: ReturnType<typeof createEmptyMindNode>, layout: 'right' = 'right') {
  const { width, height } = computeMindMapLayout(
    root,
    layout,
...
export function createMindmapBoardWhiteboard(title = '未命名思维导图'): WhiteboardJSON {
  ...
  const root = createEmptyMindNode('');
