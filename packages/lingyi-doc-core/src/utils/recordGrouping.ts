import type { CellValue, ColumnDef, GroupRule, RecordRow, SortRule } from '../types/index';
import { isColumnHidden } from './columnLayout';
import { getSelectDisplayName } from './selectOptions';
...
function compareGroupKeys(
  ...
  let cmp = 0;
  if (columnDef.type === 'number' || columnDef.type === 'date' || columnDef.type === 'datetime') {
    cmp = Number(a) - Number(b);
  } else if ((columnDef.type === 'select' || columnDef.type === 'multiSelect') && columnDef.options) {
    const idxA = columnDef.options.findIndex(o => o.id === a);
    const idxB = columnDef.options.findIndex(o => o.id === b);
