...
function writeListTextEl(textEl: HTMLElement, text: string, marks?: TextMark[]): void {
  ...
  if (normalized) {
    if (normalized.includes('\n') || itemMarks.length) {
      textEl.innerHTML = marksToHtml(normalized, itemMarks);
    } else {
...
export function syncListDom(
  ...
  block.items.forEach((item, i) => {
    ...
    writeListTextEl(textSpan, item.text || '', item.marks);
    if (item.checked) {
