...
import { resetStandardRenderConfig, resolveColumnWidth, computeFreeformEffectiveRowHeights } from '@lingyi-doc/core-sheet';
import { getSheetCellEditText, DEFAULT_ROW_HEIGHT, getCellText, cellRefLabel, type FreeformSheetModel } from '@lingyi-doc/core-types';
import { resolveSheetCommentCells } from '@lingyi-doc/core-doc';
import type { CellCoord, CellValue, ColumnDef } from '@lingyi-doc/core-types';
...
export const FreeformGridView: React.FC<FreeformGridViewProps> = ({
  ...
  const findHighlightOpen = useSheetStore(s => s.findHighlightOpen);
  const findActiveIndex = useSheetStore(s => s.findActiveIndex);
  const findMatches = useSheetStore(s => s.findMatches);
  useEffect(() => {
    dirtyTrackerRef.current.markFullRedraw();
    scheduleRender();
  }, [findHighlightOpen, findActiveIndex, findMatches, dirtyTrackerRef, scheduleRender]);
  ...
  const performRender = useCallback(() => {
    ...
    const renderCtx: SheetRenderPassContext = {
      ...
      hoveredCol,
      activeHoverRow: hoveredRowRef.current,
      cornerHovered,
      fillPreviewRange,
      discreteAxisCols,
      discreteAxisRows,
  ...
  const scrollToCellRequest = useSheetStore(s => s.scrollToCellRequest);
  useEffect(() => {
    if (previewMode || !scrollToCellRequest || !viewportRef.current) return;
    const { row, col } = scrollToCellRequest;
    const vp = viewportRef.current;
    const rowHeights = resolveActiveRowHeights();
    const rect = vp.getCellRect({ row, col }, sheet.columnWidths, rowHeights);
    const { width: cw, height: ch } = containerSize;
    if (cw <= 0 || ch <= 0) {
      useSheetStore.getState().clearScrollToCellRequest();
      return;
    }

    let nextTop = vp.scrollTop;
    let nextLeft = vp.scrollLeft;
    const headerH = vp.config.headerHeight;
    const headerW = vp.config.headerWidth;
    const pad = 8;

    if (rect.y < headerH + pad) {
      nextTop -= headerH + pad - rect.y;
    } else if (rect.y + rect.height > ch - pad) {
      nextTop += rect.y + rect.height - (ch - pad);
    }
    if (rect.x < headerW + pad) {
      nextLeft -= headerW + pad - rect.x;
    } else if (rect.x + rect.width > cw - pad) {
      nextLeft += rect.x + rect.width - (cw - pad);
    }

    if (nextTop !== vp.scrollTop || nextLeft !== vp.scrollLeft) {
  ...
  return (
    ...
    >
      ...
      <SheetSharedOverlays
        ...
        displayRowHeights={effectiveRowHeights}
        containerSize={containerSize}
