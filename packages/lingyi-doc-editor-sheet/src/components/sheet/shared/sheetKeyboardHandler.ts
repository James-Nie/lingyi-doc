  ...
  const {
    ...
    setCopiedRange,
      ...
      if (e.key.length === 1 && !e.ctrlKey && !e.metaKey && !e.altKey) {
        ...
        if (activeCell) {
          e.preventDefault();
          const recordCoord = mapCoordToRecord(activeCell) ?? activeCell;
          if (isBaseSheet) {
            const colType = columnDefs[recordCoord.col]?.type;
            if (colType && isSystemColumnType(colType)) {
              store.setStatusText('系统字段不可编辑');
              return;
            }
          }
          // 清空内容但保留样式，再进入编辑；首字符写入 formulaBar，供编辑器初始化
          t.clearCellContent(recordCoord.row, recordCoord.col);
          store.markKeyboardEditOpened();
