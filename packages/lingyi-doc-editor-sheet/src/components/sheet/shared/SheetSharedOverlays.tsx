...
import { FILL_HANDLE_SIZE, getFillHandleAnchor, shouldShowFillHandle, type FreeTable } from '@lingyi-doc/core-sheet';
...
import { SheetScrollbars } from './SheetScrollbars';
...
export interface SheetSharedOverlaysProps {
  ...
  /** 滚动内容行数（分组视图可能与 sheet.rowCount 不同） */
  scrollRowCount?: number;
  /** 统一滚动入口（viewport + 合并写 store） */
  applyScroll: (scrollTop: number, scrollLeft: number) => void;
}
...
export const SheetSharedOverlays: React.FC<SheetSharedOverlaysProps> = ({
  ...
  const effectiveScrollRows = scrollRowCount ?? sheetRowCount;
  ...
  return (
    <>
      ...
      <SheetScrollbars
        ...
        rowCount={effectiveScrollRows}
        colCount={sheetColCount}
        columnWidths={sheetColumnWidths}
        rowHeights={displayRowHeights}
