...
export function handleSheetMouseMove(e: React.MouseEvent, d: SheetInteractionDeps) {
  const {
    ...
    if (!isDraggingRef.current) {
      ...
      } else if (hoverCoord && hoverColDef?.type === 'progress') {
        setHoverRatingCell(null);
        setHoverProgressCell({ row: hoverCoord.row, col: hoverCoord.col });
        const canvasEl = canvasContainerRef.current?.querySelector('canvas');
        if (canvasEl instanceof HTMLElement) canvasEl.style.cursor = 'pointer';
      } else {
        setHoverRatingCell(null);
        setHoverProgressCell(null);
      }
