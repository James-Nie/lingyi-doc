...
import type { CellCoord, CellRange, CellStyle, BaseViewType, BorderStyle } from '@lingyi-doc/core-types';
import { DEFAULT_BORDER_SIDE } from '@lingyi-doc/core-sheet';
...
interface SheetStoreState {
  ...
  /** 多维表编辑对应的记录行坐标（display ≠ record 时由 startCellEdit 写入） */
  editingRecordCoord: CellCoord | null;
  ...
  /** 查找替换：匹配单元格高亮 */
  findHighlightOpen: boolean;
  findMatches: CellCoord[];
  findActiveIndex: number;
  /** 请求滚动到指定单元格（seq 用于重复定位同一格） */
  scrollToCellRequest: { row: number; col: number; seq: number } | null;
  ...
  setEditingCell: (coord: CellCoord | null, recordCoord?: CellCoord | null) => void;
  ...
  setFindHighlights: (open: boolean, matches?: CellCoord[], activeIndex?: number) => void;
  setFindActiveIndex: (index: number) => void;
  requestScrollToCell: (row: number, col: number) => void;
  clearScrollToCellRequest: () => void;
}
...
export const useSheetStore = create<SheetStoreState>((set, get) => ({
  ...
  axisDiscreteCols: [],
  axisDiscreteRows: [],
  findHighlightOpen: false,
  findMatches: [],
  findActiveIndex: 0,
  scrollToCellRequest: null,
  ...
  setEditingCell: (coord, recordCoord) => set({
    editingCell: coord,
    editingRecordCoord: coord ? (recordCoord ?? coord) : null,
  }),
  ...
  setFindHighlights: (open, matches = [], activeIndex = 0) =>
    set({
      findHighlightOpen: open,
      findMatches: open ? matches : [],
      findActiveIndex: open ? activeIndex : 0,
    }),

  setFindActiveIndex: (index) => set({ findActiveIndex: index }),

  requestScrollToCell: (row, col) => set(state => ({
    scrollToCellRequest: {
      row,
      col,
      seq: (state.scrollToCellRequest?.seq ?? 0) + 1,
    },
  })),

  clearScrollToCellRequest: () => set({ scrollToCellRequest: null }),
}));
