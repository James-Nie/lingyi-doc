...
import type { CellValue, FormulaError } from '@lingyi-doc/core-types';
import { getRawValue } from '@lingyi-doc/core-types';
...
export class RecalcEngine {
  ...
  recalcOnChange(
    ...
    return this.recalcOnChanges([{ row: changedRow, col: changedCol }], table);
  }

  /**
   * 批量单元格变更后一次性重算依赖公式（粘贴等高吞吐场景）。
   */
  recalcOnChanges(
    ...
    const seeds = changed.map(c => toKey(c.row, c.col));
    const { order } = this._depGraph.computeRecalcOrder(seeds);
