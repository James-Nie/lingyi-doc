...
import type { ColumnDef, RecordRow, DataValidation, SelectOption } from '@lingyi-doc/core-types';
...
import type { ActiveSheetType, BaseSheetModel, FreeformSheetModel } from '@lingyi-doc/core-types';
...
import {
  ...
  writeSystemFieldsForRow,
} from './base/BaseRecordOps';
import {
  backfillSystemFieldColumn,
  isSystemColumnType,
  syncSystemFieldCellsForRow,
} from '../utils/systemFields';
import {
...
export class FreeTable {
  ...
  /** >0 时合并通知并延迟公式重算（runBatch / beginUndoBatch） */
  private _notifySuspendDepth = 0;
  /** undefined=无待发送；null=整表；CellRange=脏区 */
  private _pendingNotifyRange: CellRange | null | undefined = undefined;
  private _deferredRecalcKeys = new Set<string>();
  ...
  /** 获取普通表格列筛选条件 */
  getColumnFilters(): import('@lingyi-doc/core-types').ColumnFilterCondition[] {
  ...
  /** 设置普通表格列筛选条件 */
  setColumnFilters(conditions: import('@lingyi-doc/core-types').ColumnFilterCondition[]): void {
    if (!isFreeformSheet(this._sheet)) return;
  ...
  setColumnFilterForCol(col: number, condition: import('@lingyi-doc/core-types').ColumnFilterCondition | null): void {
    if (!isFreeformSheet(this._sheet)) return;
  ...
  private _emitChange(range: CellRange | null): void {
    for (const listener of this._changeListeners) {
      try { listener(range); } catch (e) { console.error(e); }
    }
  }

  private _coalescePendingNotify(range: CellRange | null): void {
    if (this._pendingNotifyRange === undefined) {
      this._pendingNotifyRange = range;
      return;
    }
    if (this._pendingNotifyRange === null || range === null) {
      this._pendingNotifyRange = null;
      return;
    }
    const a = this._pendingNotifyRange;
    const b = range;
    if (a.sheetId !== b.sheetId) {
      this._pendingNotifyRange = null;
      return;
    }
    this._pendingNotifyRange = {
      sheetId: a.sheetId,
      start: {
        row: Math.min(a.start.row, a.end.row, b.start.row, b.end.row),
        col: Math.min(a.start.col, a.end.col, b.start.col, b.end.col),
      },
      end: {
        row: Math.max(a.start.row, a.end.row, b.start.row, b.end.row),
        col: Math.max(a.start.col, a.end.col, b.start.col, b.end.col),
      },
    };
  }

  private _applyRecalcUpdates(
    updates: Array<{ row: number; col: number; value: import('@lingyi-doc/core-types').CellValue }>,
  ): void {
  ...
  private _flushSuspendedSideEffects(): void {
    if (this._deferredRecalcKeys.size > 0) {
      const seeds = Array.from(this._deferredRecalcKeys, key => {
        const { row, col } = keyToCoord(key);
        return { row, col };
      });
      this._deferredRecalcKeys.clear();
      try {
        this._applyRecalcUpdates(this._recalcEngine.recalcOnChanges(seeds, this));
      } catch (e) {
        console.error('Recalc error:', e);
      }
    }
    if (this._pendingNotifyRange !== undefined) {
      const range = this._pendingNotifyRange;
      this._pendingNotifyRange = undefined;
      this._emitChange(range);
    }
  }

  private _notifyChange(range: CellRange | null): void {
    if (this._notifySuspendDepth > 0) {
      this._coalescePendingNotify(range);
      return;
    }
    this._emitChange(range);
  }

  /** 多维表：记录字段级变更历史，并同步更新人/更新时间 */
  private _maybeRecordFieldChange(
    row: number,
    col: number,
    before: import('@lingyi-doc/core-types').CellValue,
    after: import('@lingyi-doc/core-types').CellValue,
    options?: SetCellOptions,
  ): void {
    if (!isBaseSheet(this._sheet)) return;
    // 写入系统字段自身时跳过，避免递归
    if (isSystemColumnType(this._sheet.columnDefs[col]?.type) && options?.skipHistory) {
      return;
    }
    maybeRecordFieldChange(
      this._sheet,
      ...
      (r) => {
        writeSystemFieldsForRow(
          this._sheet as import('@lingyi-doc/core-types').BaseSheetModel,
          r,
          (rr, cc, value) => this.setCellValue(rr, cc, value, { skipHistory: true }),
          true,
        );
      },
    );
  ...
  /** 新增系统字段列后，按行元数据回填整列 */
  backfillSystemFieldColumn(colIndex: number): void {
    if (!isBaseSheet(this._sheet)) return;
    this.ensureRowRecords();
    backfillSystemFieldColumn(
      this._sheet,
      colIndex,
      (row, col, value) => this.setCellValue(row, col, value, { skipHistory: true }),
    );
  }

  /** 将行元数据同步到系统字段单元格 */
  syncSystemFieldsForRow(row: number, onlyUpdated = false): void {
    if (!isBaseSheet(this._sheet)) return;
    this.ensureRowRecords();
    writeSystemFieldsForRow(
      this._sheet,
      row,
      (r, c, value) => this.setCellValue(r, c, value, { skipHistory: true }),
      onlyUpdated,
    );
  }
  ...
  private _triggerRecalc(row: number, col: number): void {
    if (this._notifySuspendDepth > 0) {
      this._deferredRecalcKeys.add(coordToKey({ row, col }));
      return;
    }
    try {
      this._applyRecalcUpdates(this._recalcEngine.recalcOnChange(row, col, this));
    } catch (e) {
  ...
  setCell(
    ...
    if (
      isBaseSheet(this._sheet) &&
      isSystemColumnType(this._sheet.columnDefs[col]?.type) &&
      !options?.skipHistory
    ) {
      return;
    }
    ...
    let cellValue: import('@lingyi-doc/core-types').CellValue;
    if (formula) {
  ...
  setCellValue(
    row: number,
    col: number,
    cellValue: import('@lingyi-doc/core-types').CellValue,
    options?: SetCellOptions,
  ): void {
    // 系统字段仅允许内部回写（skipHistory），禁止用户编辑
    if (
      isBaseSheet(this._sheet) &&
      isSystemColumnType(this._sheet.columnDefs[col]?.type) &&
      !options?.skipHistory
  ...
  /**
   * 原子替换单元格（值 + 样式一次写入）。next 为 null 表示删除。
   * 用于粘贴等批量场景，避免 clearCell + setCell + setCellStyle 三次开销。
   */
  replaceCell(row: number, col: number, next: CellData | null, options?: SetCellOptions): void {
    if (
      isBaseSheet(this._sheet) &&
      isSystemColumnType(this._sheet.columnDefs[col]?.type) &&
      !options?.skipHistory
    ...
    let nextStored: CellData | null = null;
    if (next) {
      let finalValue = next.value;
      if (finalValue.type === 'formula') {
        try {
          finalValue = this._recalcEngine.evaluateAndStore(finalValue.formula, this, row, col);
        } catch {
          // Keep as-is
        }
      }
      nextStored = { value: finalValue };
      if (next.style) {
        nextStored.style = { ...next.style };
      }
    }

    if (nextStored) {
      this._sheet.cells.set(key, nextStored);
    } else {
      this._sheet.cells.delete(key);
    }

    const stored = nextStored;
    this._pushUndo({
      type: 'replaceCell',
      undo: () => {
        if (oldValue) this._sheet.cells.set(key, oldValue);
      ...
      redo: () => {
        if (stored) this._sheet.cells.set(key, stored);
        else this._sheet.cells.delete(key);
        this._notifyChange(this._makeRange(row, col, row, col));
      },
    });
    ...
    this._triggerRecalc(row, col);
    this._maybeRecordFieldChange(
      row,
      col,
      oldValue?.value ?? { type: 'empty' },
      stored?.value ?? { type: 'empty' },
      options,
    );
    this._notifyChange(this._makeRange(row, col, row, col));
  }
  ...
  /**
   * 批量替换单元格：单条 undo 快照，避免逐格 Operation 导致千行粘贴内存暴涨/卡死。
   */
  replaceCellsBulk(
    entries: Array<{ row: number; col: number; cell: CellData | null }>,
  ): void {
    if (entries.length === 0) return;

    const hasMerges = isFreeformSheet(this._sheet) && this._sheet.mergeRanges.length > 0;
    const before: Array<[string, CellData | undefined]> = [];
    const after: Array<[string, CellData | null]> = [];
    let minRow = Infinity;
    let maxRow = -1;
    let minCol = Infinity;
    let maxCol = -1;
    ...
    for (const entry of entries) {
      ...
      if (hasMerges && this._findMergeConflict(row, col)) continue;

      const key = coordToKey({ row, col });
      const oldValue = this._sheet.cells.get(key);
      if (!oldValue && !entry.cell) continue;
      ...
      if (entry.cell) {
        ...
        let finalValue = entry.cell.value;
        if (finalValue.type === 'formula') {
          try {
            finalValue = this._recalcEngine.evaluateAndStore(finalValue.formula, this, row, col);
          } catch {
        ...
        nextStored = { value: finalValue };
        if (entry.cell.style) {
          nextStored.style = { ...entry.cell.style };
        }
      } else if (oldValue?.value.type === 'formula') {
        this._recalcEngine.removeFormula(row, col);
      }

      before.push([key, oldValue]);
      after.push([key, nextStored]);

      if (nextStored) this._sheet.cells.set(key, nextStored);
      else this._sheet.cells.delete(key);
    ...
    const notifyRange = this._makeRange(minRow, minCol, maxRow, maxCol);
    this._pushUndo({
      type: 'replaceCellsBulk',
      undo: () => {
        for (const [key, old] of before) {
          if (old) this._sheet.cells.set(key, old);
          else this._sheet.cells.delete(key);
        }
        this._notifyChange(notifyRange);
      },
      redo: () => {
        for (const [key, next] of after) {
          if (next) this._sheet.cells.set(key, next);
          else this._sheet.cells.delete(key);
        }
        this._notifyChange(notifyRange);
      },
  ...
  /**
   * 大表粘贴：矩形清空 + 稀疏写入 + 单条 undo。
   * 按行分片让出主线程，避免 3000+ 行同步写导致页面卡死。
   */
  async applyPasteRect(options: {
    startRow: number;
    startCol: number;
    rows: number;
    cols: number;
    /** 相对起点的非空单元格 */
    cells: Array<{ relRow: number; relCol: number; cell: CellData }>;
    chunkRows?: number;
  }): Promise<void> {
    const {
      startRow,
    ...
    const hasMerges = isFreeformSheet(this._sheet) && this._sheet.mergeRanges.length > 0;
    const skipRecalc = this._recalcEngine.formulaCount === 0;
    const overwritten: Array<[string, CellData]> = [];
    const written: Array<[string, CellData]> = [];
    const rectSize = rows * cols;
    // 稀疏表：按 Map 扫比按矩形扫更省；密表仍走矩形
    const clearViaMap = this._sheet.cells.size > 0 && this._sheet.cells.size < rectSize;

    const clearOne = (key: string, row: number, col: number, old: CellData) => {
      if (
        isBaseSheet(this._sheet) &&
        isSystemColumnType(this._sheet.columnDefs[col]?.type)
      ) {
      ...
      if (hasMerges && this._findMergeConflict(row, col)) return;
      overwritten.push([key, old]);
      if (old.value.type === 'formula') {
        this._recalcEngine.removeFormula(row, col);
      }
      this._sheet.cells.delete(key);
    };

    // 1) 清空目标矩形内已有单元格（对应剪贴板空格覆盖）
    if (clearViaMap) {
      const toClear: Array<[string, number, number, CellData]> = [];
      for (const [key, old] of this._sheet.cells) {
        const { row, col } = keyToCoord(key);
        if (row < startRow || row > endRow || col < startCol || col > endCol) continue;
        toClear.push([key, row, col, old]);
      }
      for (let i = 0; i < toClear.length; i++) {
        const [key, row, col, old] = toClear[i];
        clearOne(key, row, col, old);
        if (chunkRows > 0 && (i + 1) % Math.max(400, chunkRows * 3) === 0) {
          await yieldToMain();
        }
      }
    } else {
      for (let r = startRow; r <= endRow; r++) {
        for (let c = startCol; c <= endCol; c++) {
          ...
          const old = this._sheet.cells.get(key);
          if (!old) continue;
          clearOne(key, r, c, old);
        }
        if (chunkRows > 0 && (r - startRow + 1) % chunkRows === 0) {
          await yieldToMain();
        }
      }
    }
    ...
    for (let i = 0; i < cells.length; i++) {
      const { relRow, relCol, cell } = cells[i];
      ...
      const stored: CellData = { value: finalValue };
      if (cell.style) stored.style = { ...cell.style };

      this._sheet.cells.set(key, stored);
      written.push([key, stored]);
      if (!skipRecalc) this._triggerRecalc(row, col);
    ...
    const notifyRange = this._makeRange(startRow, startCol, endRow, endCol);
    this._pushUndo({
      type: 'pasteRect',
      undo: () => {
        for (const [key] of written) this._sheet.cells.delete(key);
        for (const [key, old] of overwritten) this._sheet.cells.set(key, old);
        this._notifyChange(notifyRange);
      },
      redo: () => {
        for (const [key] of overwritten) this._sheet.cells.delete(key);
        for (const [key, next] of written) this._sheet.cells.set(key, next);
        this._notifyChange(notifyRange);
      },
    });
    this._notifyChange(notifyRange);
  ...
  clearCellContent(row: number, col: number): void {
    if (isBaseSheet(this._sheet) && isSystemColumnType(this._sheet.columnDefs[col]?.type)) {
      return;
    }
    const key = coordToKey({ row, col });
  ...
  insertRows(index: number, count: number = 1): void {
    if (index < 0 || index > this._sheet.rowCount || count <= 0) return;

    const appendOnly = index === this._sheet.rowCount;
    if (!appendOnly) {
      // 移动现有数据
      const newCells = new Map<string, CellData>();
      for (const [key, cell] of this._sheet.cells) {
  ...
  insertColumns(index: number, count: number = 1): void {
    if (index < 0 || index > this._sheet.colCount || count <= 0) return;

    const appendOnly = index === this._sheet.colCount;
    if (!appendOnly) {
      const newCells = new Map<string, CellData>();
      for (const [key, cell] of this._sheet.cells) {
        const coord = keyToCoord(key);
  ...
  /** 将 fn 内多次变更合并为一条撤销记录，并静默合并 onChange / 延迟公式重算 */
  runBatch(fn: () => void, type = 'batch'): void {
    this._notifySuspendDepth += 1;
    try {
      this._undo.runBatch(fn, type, r => this._notifyChange(r));
    } finally {
      this._notifySuspendDepth -= 1;
      if (this._notifySuspendDepth === 0) {
        this._flushSuspendedSideEffects();
      }
    }
  }
  ...
  toJSON(): object {
    ...
    // 视图选中状态仅会话有效：落库时固定为表格（表单除外，兼容问卷）
    const views = this._sheet.views;
    const sheetWithView = this._sheet as typeof this._sheet & { activeViewId?: string };
    const currentView = views?.find(v => v.viewId === sheetWithView.activeViewId);
    const persistActiveViewId = currentView?.viewType === 'form'
      ? currentView.viewId
      : (views?.find(v => v.viewType === 'grid')?.viewId ?? sheetWithView.activeViewId);
