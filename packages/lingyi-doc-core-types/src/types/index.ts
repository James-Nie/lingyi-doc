...
/** A1 样式单元格引用：row/col 均为 0-based → "A1" */
...
export type NumberFormat =
  ...
  | { kind: 'currency'; symbol: string; decimals: number; symbolPosition?: 'prefix' | 'suffix' }
...
export function formatNumber(value: number, fmt: NumberFormat = { kind: 'general' }): string {
  ...
  switch (fmt.kind) {
    ...
    case 'currency': {
      ...
      const sign = value < 0 ? '-' : '';
      if (fmt.symbolPosition === 'suffix') {
        return `${sign}${numText}${fmt.symbol}`;
      }
      return `${sign}${fmt.symbol}${numText}`;
    }
    case 'percent':
...
export function parseFieldValue(
  ...
  switch (columnType) {
    ...
    case 'progress': {
      ...
      if (!isNaN(num)) {
        let format: NumberFormat = { kind: 'general' };
        if (columnType === 'currency') {
          const symbol = columnDef?.currencySymbol || '¥';
          const decimals = typeof columnDef?.currencyPrecision === 'number'
            ? Math.max(0, Math.min(10, Math.round(columnDef.currencyPrecision)))
            : 2;
          const symbolPosition = columnDef?.currencySymbolAlign === 'right' ? 'suffix' : 'prefix';
          format = { kind: 'currency', symbol, decimals, symbolPosition };
    ...
    case 'multilineText':
      return { type: 'text', text: input.replace(/\r\n/g, '\n') };
...
export function resolveColumnDefaultValue(
  columnDef: Pick<ColumnDef, 'type' | 'defaultValue' | 'currencySymbol' | 'currencySymbolAlign' | 'currencyPrecision'>,
): CellValue | null {
  ...
  switch (type) {
    ...
    case 'updatedTime': {
      ...
      if (!raw) return null;
      return parseFieldValue(raw, type, columnDef);
    }
    ...
    case 'progress': {
      ...
      return parseFieldValue(String(num), type, columnDef);
    }

    default:
      return parseFieldValue(String(defaultValue), type, columnDef);
  }
}
...
export type ColumnType =
  | 'text' | 'multilineText' | 'number' | 'currency' | 'percent' | 'date' | 'datetime'
...
export type {
  ...
  ChartAxisStyle,
  ChartGridStyle,
  DashboardChartDisplayConfig,
  MetricNumberFormat,
  MetricLargeNumberAbbrev,
  DashboardMetricCardConfig,
  DashboardRankListConfig,
  DashboardGridViewConfig,
  DashboardProgressConfig,
  DashboardProgressShape,
  DashboardProgressUnit,
  DashboardTextConfig,
