...
import type { ConnectorElement, MindmapElement, MindmapLayout, ShapeElement, ShapeKind, WhiteboardElement, WhiteboardPoint, WhiteboardViewport } from '@lingyi-doc/core-whiteboard';
import type { DocCommentThread } from '@lingyi-doc/core-doc';
import type { MindNode, MindNoteBranchStyle } from '@lingyi-doc/core-types';
import { cloneWhiteboardElement, connectorOptsFromLineShapeKind, genWhiteboardId, nextZIndex } from '@lingyi-doc/core-whiteboard';
import { deleteMindNode, duplicateMindNode, insertMindChild, insertMindParent, insertMindSibling, remapMindmapRootForLayout, toggleMindCollapse, updateMindNode, WHITEBOARD_MIND_BRANCH_STYLE_DEFAULT } from '@lingyi-doc/core-mindmap';
...
import { ensureElementsAboveSections } from './sectionUtils';
import { expandIdsWithContainerContents, ensureElementsAboveTables } from './tableContainerUtils';
import {
  applyShapeStyle,
  canCopyStyle,
  canTransformElement,
...
import { validateImageFile } from '@lingyi-doc/editor-shared';
...
export const WhiteboardEditor: React.FC<WhiteboardEditorProps> = ({
  ...
  const commitElements = useCallback((next: WhiteboardElement[], recordHistory?: boolean) => {
    onElementsChangeProp(
      ensureElementsAboveTables(ensureElementsAboveSections(next)),
      recordHistory,
    );
  }, [onElementsChangeProp]);
  ...
  const commitElementUpdate = useCallback((
    ...
    const normalized = ensureElementsAboveTables(ensureElementsAboveSections(merged));
  ...
  const nudgeSelection = useCallback((dx: number, dy: number) => {
    ...
    const expanded = expandIdsWithContainerContents(selectedIds, elements);
    const idSet = new Set(
      expanded.filter(id => !elements.find(e => e.id === id)?.locked),
    );
  ...
  const deleteSelection = useCallback(() => {
    ...
    const expandedIds = expandIdsWithContainerContents(selectedIds, elements);
  ...
  const duplicateSelection = useCallback((offsetX = 16, offsetY = 16) => {
    if (!selectedIds.length || readOnly) return;
    const idSet = new Set(expandIdsWithContainerContents(selectedIds, elements));
  ...
  const copySelection = useCallback(async () => {
    ...
    try {
      await writeWhiteboardClipboard(elements, [...idSet]);
    } catch {
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      // 单选可编辑对象时，可打印键交给 Canvas 进入内联编辑，避免 t/n/v 等工具快捷键抢键
      if (!e.metaKey && !e.ctrlKey && !e.altKey && e.key.length === 1 && e.key !== ' ') {
        const singleId = selectedIds.length === 1 ? selectedIds[0] : null;
        const singleEl = singleId ? elements.find(el => el.id === singleId) : null;
        if (
          singleEl
          && !singleEl.locked
          && (singleEl.type === 'shape'
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0, minWidth: 0 }}>
        {!readOnly && shapeLibraryOpen && (
          <ShapeLibraryPanel
            ...
            selectedTablePreset={toolState.tablePreset}
            selectedConnector={{
              style: toolState.connectorStyle,
              strokeDash: toolState.connectorStrokeDash,
              arrowStart: toolState.connectorArrowStart,
              arrowEnd: toolState.connectorArrowEnd,
            }}
            ...
            onSelectLineConnector={kind => {
              const opts = connectorOptsFromLineShapeKind(kind);
              if (!opts) return;
              const next: WhiteboardToolState = {
                ...toolStateRef.current,
                tool: 'connector',
                connectorStyle: opts.style,
                connectorStrokeDash: opts.strokeDash ?? null,
                connectorArrowStart: opts.arrowStart ?? null,
                connectorArrowEnd: opts.arrowEnd ?? null,
                shapeKind: null,
                shapeCategoryId: null,
              };
              toolStateRef.current = next;
