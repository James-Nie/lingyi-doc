...
export function DiagramShapeIcon({ kind }: { kind: ShapeKind }): React.ReactNode {
  switch (kind) {
    ...
    // UML类图关系线 - 45度方向，箭头方向与线段方向保持一致
    case 'umlAggregation':
      // 聚合：实线 + 空心菱形（终点，沿45度方向）
      return (
        <>
          <line x1="5" y1="19" x2="12" y2="12" {...S} />
          {/* 菱形沿45度方向：顶点指向线段方向 */}
          <polygon points="12,12 15,9 12,6 9,15" {...S} />
        </>
      );
    case 'umlComposition':
      // 组合：实线 + 实心菱形（终点，沿45度方向）
      return (
        <>
          <line x1="5" y1="19" x2="12" y2="12" {...S} />
          <polygon points="12,12 15,9 12,6 9,15" fill="#333" stroke="none" />
        </>
      );
    case 'umlGeneralization':
      // 继承：实线 + 实心三角箭头（终点，沿45度方向）
      return (
        <>
          <line x1="5" y1="19" x2="12" y2="12" {...S} />
          {/* 三角形顶点指向45度方向 */}
          <polygon points="12,12 15,9 9,15" fill="#333" stroke="none" />
        </>
      );
    case 'umlDependency':
      // 依赖：虚线 + 开放箭头（终点，沿45度方向）
      return (
        <>
          <line x1="5" y1="19" x2="12" y2="12" {...S} strokeDasharray="3,2" />
          <polygon points="12,12 15,9 9,15" {...S} fill="none" />
        </>
      );
    case 'umlRealization':
      // 实现：虚线 + 三角箭头（终点，沿45度方向）
      return (
        <>
          <line x1="5" y1="19" x2="12" y2="12" {...S} strokeDasharray="3,2" />
          <polygon points="12,12 15,9 9,15" {...S} />
        </>
      );
    case 'seqActor':
      return (
    ...
    case 'dfdDataStore':
      return <path d="M5 5.5h14v13H5zM5 9h14" {...S} />;
    case 'dfdSubProcess':
      return <rect x="5" y="5" width="14" height="14" rx="0.8" {...S} />;
