...
import { computeMindMapLayout, createWhiteboardMeasureOptions, findMindNode, WHITEBOARD_MIND_BRANCH_STYLE_DEFAULT } from '@lingyi-doc/core-mindmap';
...
export function drawMindmapElement(
  ...
  for (const ln of layout.nodes) {
    ...
    if (appearance.showBox && appearance.fillColor) {
      const fillAlpha = Math.max(0, Math.min(1, (node.fillOpacity ?? 100) / 100));
      const borderAlpha = Math.max(0, Math.min(1, (node.borderOpacity ?? 100) / 100));
    ...
    const textAlign = node.textAlign ?? (appearance.showBox ? 'center' : 'left');
    ctx.textAlign = textAlign;
    ctx.textBaseline = 'middle';
    const text = node.text || '输入文本';
    if (appearance.showBox) {
      const tx = textAlign === 'left'
        ? ln.x + 8
        : textAlign === 'right'
          ? ln.x + ln.width - 8
          : ln.x + ln.width / 2;
      wrapText(ctx, text, tx, ln.y + ln.height / 2, ln.width - 16, appearance.fontSize);
    } else {
