...
/** 同级节点层级：front=首位 forward=上移 backward=下移 back=末位 */
export function moveMindSibling(
  root: MindNode,
  targetId: string,
  ...
  const parent = findMindNode(clone, found.parent.id);
  if (!parent) return root;
  const list = parent.node.children;
  const from = found.index;
  if (from < 0 || from >= list.length) return root;
  const [item] = list.splice(from, 1);
  let next = from;
  if (to === 'front') next = 0;
  else if (to === 'back') next = list.length;
  else if (to === 'forward') next = Math.max(0, from - 1);
  else next = Math.min(list.length, from + 1);
...
/** 切换左右类布局时，重映射根节点一级子节点的 branchDir。
 * - 切到单向 left/right：清空 branchDir，全部归到该布局主侧（符合「向左/向右」语义）
 * - 切到 balanced：保留原视觉侧，避免左右子树被打乱
 */
export function remapMindmapRootForLayout(
  ...
  if (isHorizontalMindLayout(fromLayout) && isHorizontalMindLayout(toLayout)) {
    // 单向布局：全部归主侧，避免向右→向左后子节点仍停在右侧、折叠/连线朝左
    if (toLayout === 'left' || toLayout === 'right') {
      return {
        ...root,
        children: root.children.map(child => remapRootChildBranchDir(child, undefined)),
      };
    }
    return {
      ...root,
