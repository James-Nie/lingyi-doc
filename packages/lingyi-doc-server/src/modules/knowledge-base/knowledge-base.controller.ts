...
export class KnowledgeBaseController {
  ...
  async create(@CurrentUser() user: AuthUser, @Body() body: Record<string, unknown>) {
    try {
      ...
      // 不强制写死默认值；缺省/未知值交给 service 按 scope 派生：
      //   - 企业 KB（scope=2）默认 organization，让租户成员能直接看到
      //   - 个人 KB（scope=1）默认 members，仅本人可见
      const rawVisibility = body.visibility;
      const visibility: KnowledgeBaseVisibility | undefined =
        rawVisibility === 'organization' || rawVisibility === 'members'
          ? rawVisibility
          : undefined;
