...
export class ConsumerAuthService {
  ...
  async sendSms(body: Record<string, unknown>, req: Request) {
    ...
    if (scene === 'register') {
      ...
      const existing = await this.userRepository.findByPhone(normalized);
      if (!existing) {
        throw new BusinessException(120001, '该手机号未注册');
      }
    }
  ...
  async login(body: Record<string, unknown>, req: Request) {
    const { password, account, email, loginType } = body ?? {};
    const loginAccount = String(account ?? email ?? '').trim();
    if (!loginAccount || !password) {
      throw new BusinessException(100002, '缺少必填参数');
    }
    if (loginType !== undefined && !['email', 'phone', 'auto'].includes(String(loginType))) {
      throw new BusinessException(100002, '无效的登录方式');
    }
    ...
    try {
      return await this.authHelpers.loginUser({
        ...
        audience: 'consumer',
        req,
        loginType: loginType === undefined ? 'auto' : (String(loginType) as 'email' | 'phone' | 'auto'),
      });
    } catch (err) {
      if (err instanceof AuthError) {
        throw new BusinessException(err.code, err.message, authErrorStatus(err.code));
  ...
  async refresh(body: Record<string, unknown>, req: Request) {
    ...
    try {
      this.authService.verifyRefreshToken(String(refreshToken));
  ...
  async me(user: AuthUser) {
    ...
    const adminRoles = await this.adminRoleRepository.getUserRoles(dbUser.id);
    return {
      ...this.userRepository.toPublicUser(dbUser),
      adminRoles,
      adminPermissions: await this.adminRoleRepository.getUserPermissions(dbUser.id),
  ...
  async resetPassword(body: Record<string, unknown>, req: Request) {
    ...
    const normalized = this.smsVerification.assertValidPhone(String(phone));
    ...
    await this.authSessionRepository.revokeAllForUser(user.id, 'consumer');

    this.logger.log(`user ${user.id} reset password via sms from ${this.authService.getClientIp(req)}`);
    return { success: true };
  }
}
