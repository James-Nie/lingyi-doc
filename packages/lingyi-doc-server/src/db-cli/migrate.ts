...
runMigration()
  .catch((err) => {
    ...
    if (message.includes('ECONNREFUSED') || message.includes('password authentication') || message.includes('database')) {
      console.error('[Migrate] 请确认 PostgreSQL 已启动且 .env 中数据库配置正确');
