...
export class AuthHelpersService {
  ...
  async issueTokens(
    ...
    await this.authSessionRepository.create({
      ...
      refreshToken,
      clientType: 'consumer',
      expiresAt: this.refreshExpiresAt('consumer'),
    ...
    publicUser.needChangePassword = user.need_change_password ?? false;
  ...
  async loginUser(input: {
    ...
    req: RequestMeta;
    loginType?: 'email' | 'phone' | 'auto';
  }): Promise<AuthTokenResult> {
    const account = input.account.trim();
    const loginType = input.loginType ?? 'auto';
    const normalizedPhone = account.replace(/[\s-]/g, '');

    let user: DbUser | null = null;
    if (loginType === 'phone') {
      user = await this.userRepository.findByPhone(normalizedPhone);
      if (!user) throw new AuthError(120001, '手机号未注册');
    } else if (loginType === 'email') {
      user = await this.userRepository.findByEmail(account.toLowerCase());
      if (!user) throw new AuthError(120001, '邮箱未注册');
    } else {
      user = isValidPhone(normalizedPhone)
