    ...
    if (row.kbSlug == null) {
      const kbRow = await this.docRepo.query(
        `SELECT kb.id FROM kb_nodes kn
         ...
         WHERE kn.doc_id = $1 AND kn.is_deleted = false LIMIT 1`,
        [docId],
      ) as Array<{ id: string }>;
      if (kbRow[0]?.id) {
  ...
  async resolveDocIdByPath(
    ...
    const rows = await this.docRepo.query(
      ...
      WHERE d.doc_slug = $1
        ...
        AND (
          (d.scope = 2 AND t.space_slug = $2 AND (kb.kb_slug = $3 OR (kb.kb_slug IS NULL AND t.default_book_slug = $4)))
          OR (d.scope = 1 AND u.personal_space_slug = $5 AND (kb.kb_slug = $6 OR (kb.kb_slug IS NULL AND u.default_book_slug = $7)))
        )
      LIMIT 1`,
