...
function toDbUser(entity: UserEntity): DbUser {
  return {
    ...
    need_change_password: entity.needChangePassword ?? false,
...
function toPublicUser(user: DbUser, defaultUserSource: UserSource): PublicUser {
  return {
    ...
    status: user.status,
    needChangePassword: user.need_change_password ?? false,
...
export class UserRepository {
  ...
  async findByPhoneOrEmail(phone?: string, email?: string): Promise<DbUser | null> {
    if (phone) {
  ...
  async create(input: {
    ...
    await this.repo.save({
      ...
      canCreateTeam: 0,
      needChangePassword: input.needChangePassword ?? false,
    });
  ...
  async updatePassword(id: string, passwordHash: string): Promise<void> {
    await this.repo.update(id, { passwordHash, needChangePassword: false });
  }
  ...
  async countDailyActiveConsumers(since: Date, until: Date): Promise<Map<string, number>> {
    const rows = await this.repo
      .createQueryBuilder('u')
      .select("to_char(u.\"last_login_at\", 'YYYY-MM-DD')", 'day')
      .addSelect('COUNT(*)', 'cnt')
      .where('u.userType = :userType', { userType: 'consumer' })
      .andWhere('u.lastLoginAt >= :since', { since })
      .andWhere('u.lastLoginAt <= :until', { until })
      .groupBy("to_char(u.\"last_login_at\", 'YYYY-MM-DD')")
