...
const DEFAULT_GROUPS: Array<{ name: string; positions: string[] }> = [
  {
    name: '技术',
    positions: ['技术总监', '架构师', '前端工程师', '开发工程师', '测试工程师', '运维工程师'],
  },
  {
    name: '产品',
    positions: ['产品总监', '产品经理', '产品助理', '设计师'],
  },
];
...
export class PositionRepository {
  ...
  async ensureDefaults(tenantId: string): Promise<void> {
    const count = await this.groupRepo.count({ where: { tenantId } });
    if (count > 0) return;

    for (let gi = 0; gi < DEFAULT_GROUPS.length; gi += 1) {
      ...
      for (let pi = 0; pi < g.positions.length; pi += 1) {
        await this.positionRepo.save({
          id: uuidv4(),
          tenantId,
          groupId,
          name: g.positions[pi],
          avatarKey: `avatar_${pi % 18}`,
          sortOrder: pi,
        });
      }
    }
