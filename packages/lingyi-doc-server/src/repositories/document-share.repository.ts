...
export class DocumentShareRepository {
  ...
  async upsertShare(input: {
    ...
    if (existing) {
      ...
      existing.allowPrint = input.allowPrint;
      existing.allowCopy = input.allowCopy;
      existing.allowReshare = input.allowReshare;
      existing.watermarkEnabled = input.watermarkEnabled;
  ...
  async listSharedWithUser(
    ...
    const orderClause = sortBy === 'created'
      ...
      : sortBy === 'updated'
        ? 'dc.updated_at'
        : 'COALESCE(d.last_visited_at, dc.updated_at)';
    ...
    const rows = await this.shareUserRepo.query(
      ...
      INNER JOIN documents d ON d.id = su.doc_id AND d.is_deleted = false
      LEFT JOIN document_contents dc ON dc.doc_id = d.id
      LEFT JOIN users u ON d.owner_id = u.id
      LEFT JOIN users g ON su.granted_by = g.id
      LEFT JOIN tenants t ON d.tenant_id = t.id
      LEFT JOIN kb_nodes kn ON kn.doc_id = d.id AND kn.is_deleted = false
      LEFT JOIN knowledge_bases kb ON kb.id = kn.kb_id AND kb.is_deleted = false
  ...
  async backfillCollaboratorsFromShareVisits(userId: string): Promise<void> {
    const candidates = await this.visitLogRepo.query(
      ...
      INNER JOIN documents d ON d.id = v.doc_id AND d.is_deleted = false
  ...
  async getVisitStats(docId: string): Promise<{
    ...
    const rows = await this.visitLogRepo.query(
      `SELECT
        ...
        COUNT(*) AS "visitCount",
        SUM(CASE WHEN created_at >= CURRENT_DATE AND created_at < CURRENT_DATE + INTERVAL '1 day' THEN 1 ELSE 0 END) AS "todayNewVisits"
      FROM doc_share_visit_log
      WHERE doc_id = $1 AND visit_status = 'success'`,
      [docId],
    );
    const row = rows[0] ?? {};
    return {
  ...
  async listVisitRecords(docId: string, limit = 50): Promise<Array<{
    ...
    const rows = await this.visitLogRepo.query(
      ...
      WHERE v.doc_id = $1 AND v.visit_status = 'success' AND v.visitor_id IS NOT NULL
