...
export interface DbDocumentContent {
  doc_id: string;
...
/** 多维表行级变更历史条目（随保存请求携带，服务端抽取入库，去重键为 id） */
...
/** 历史读取接口返回的单条记录 */
export interface RecordHistoryReadEntry {
  id: string;
  at: number;
  by: string;
  action: 'create' | 'update';
...
export interface RecordHistoryListResult {
  items: RecordHistoryReadEntry[];
  total: number;
  hasMore: boolean;
}
