...
/** 端点移动时平移折点：整体平移保持正交；单端移动只调整邻接折点以维持正交 */
export function shiftElbowPointsForEndpointMove(
  ...
  // 两端同位移：整体平移，保持正交
  if (Math.abs(dStart.x - dEnd.x) < EPS && Math.abs(dStart.y - dEnd.y) < EPS) {
    return points.map(p => ({ x: p.x + dStart.x, y: p.y + dStart.y }));
  }
  ...
  if (points.length < 3) return next;

  const startOnly = Math.abs(dEnd.x) < EPS && Math.abs(dEnd.y) < EPS;
  const endOnly = Math.abs(dStart.x) < EPS && Math.abs(dStart.y) < EPS;

  if (startOnly) {
    for (let i = 1; i < n; i++) {
      next[i] = { x: points[i].x + dStart.x, y: points[i].y + dStart.y };
    }
    const axis0 = segmentAxis(prevStart, points[1]);
    if (axis0 === 'h') next[1] = { x: next[1].x, y: nextStart.y };
    else if (axis0 === 'v') next[1] = { x: nextStart.x, y: next[1].y };
    return next;
  }

  if (endOnly) {
    for (let i = 1; i < n; i++) {
      next[i] = { x: points[i].x + dEnd.x, y: points[i].y + dEnd.y };
    }
    const axisN = segmentAxis(points[n - 1], prevEnd);
    if (axisN === 'h') next[n - 1] = { x: next[n - 1].x, y: nextEnd.y };
    else if (axisN === 'v') next[n - 1] = { x: nextEnd.x, y: next[n - 1].y };
    return next;
  }

  // 两端不同位移：分别钉住出/入线段轴向，其余按比例（失败时由调用方回退）
  const axis0 = segmentAxis(prevStart, points[1]);
  if (axis0 === 'h') next[1] = { x: points[1].x + dStart.x, y: nextStart.y };
  else if (axis0 === 'v') next[1] = { x: nextStart.x, y: points[1].y + dStart.y };

  const axisN = segmentAxis(points[n - 1], prevEnd);
  if (axisN === 'h') next[n - 1] = { x: points[n - 1].x + dEnd.x, y: nextEnd.y };
  else if (axisN === 'v') next[n - 1] = { x: nextEnd.x, y: points[n - 1].y + dEnd.y };

  for (let i = 2; i < n - 1; i++) {
    const t = i / n;
    next[i] = {
      x: points[i].x + dStart.x * (1 - t) + dEnd.x * t,
      y: points[i].y + dStart.y * (1 - t) + dEnd.y * t,
    };
  }
...
function prepareElbowPointsForEndpointMove(
  ...
  if (endpointsMatch(points[0], start) && endpointsMatch(points[points.length - 1], end)) {
    return isOrthogonalPath(points) ? points : [start, end];
  }
...
function orthogonalizeByInsertingCorners(points: WhiteboardPoint[]): WhiteboardPoint[] {
  if (points.length < 2) return points.map(p => ({ ...p }));
  const out: WhiteboardPoint[] = [{ ...points[0] }];
  for (let i = 1; i < points.length; i++) {
    const prev = out[out.length - 1];
    const curr = points[i];
    if (segmentAxis(prev, curr)) {
      out.push({ ...curr });
      continue;
    }
    // 对角段插入直角折点（先水平后垂直）
...
function ensureOrthogonalElbowRoute(
  ...
  const fixed = orthogonalizeByInsertingCorners(candidate);
  if (isOrthogonalPath(fixed)) {
    return simplifyOrthogonalPath(fixed, ELBOW_MERGE_EPS);
  }
...
export function listElbowSegments(points: WhiteboardPoint[]): ElbowSegment[] {
  ...
  for (let i = 0; i < points.length - 1; i++) {
    ...
    if (len < EPS) continue; // 零长度 stub 不生成手柄
    let axis = segmentAxis(a, b);
    // 近正交段吸附为 H/V，避免残留对角段无法选中
    if (!axis) {
      const dx = Math.abs(b.x - a.x);
      const dy = Math.abs(b.y - a.y);
      if (dy <= Math.max(EPS, dx * 0.08)) axis = 'h';
      else if (dx <= Math.max(EPS, dy * 0.08)) axis = 'v';
    }
    if (!axis) continue;
    out.push({
      index: i,
