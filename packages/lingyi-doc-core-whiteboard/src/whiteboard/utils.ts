...
import { getMindNodeFactory, hasMindNodeFactory } from '@lingyi-doc/core-types';
...
function normalizeElement(raw: unknown, index: number): WhiteboardElement {
  ...
  const base = {
    ...
    locked: !!e.locked,
    containerId: typeof (e as { containerId?: string }).containerId === 'string'
      ? (e as { containerId: string }).containerId
      : undefined,
  };
  ...
  switch (e.type) {
    ...
    case 'table': {
      ...
      const actualRows = normalizedCells.length || rows;
      const actualCols = normalizedCells[0]?.length ?? cols;
      const colWidths = Array.isArray((e as { colWidths?: number[] }).colWidths)
        ? (e as { colWidths: number[] }).colWidths
        : undefined;
      const rowHeights = Array.isArray((e as { rowHeights?: number[] }).rowHeights)
        ? (e as { rowHeights: number[] }).rowHeights
        : undefined;
      const cellStyles = Array.isArray((e as { cellStyles?: unknown }).cellStyles)
        ? (e as { cellStyles: (Record<string, unknown> | null | undefined)[][] }).cellStyles
        : undefined;
      return {
        ...base,
        type: 'table',
        ...
        colWidths: colWidths?.length === actualCols ? colWidths : undefined,
        rowHeights: rowHeights?.length === actualRows ? rowHeights : undefined,
        cellStyles,
        fontSize: (e as { fontSize?: number }).fontSize ?? 14,
        color: (e as { color?: string }).color ?? '#1f2329',
        fontWeight: (e as { fontWeight?: number }).fontWeight,
        fontStyle: (e as { fontStyle?: 'normal' | 'italic' }).fontStyle,
        textAlign: (e as { textAlign?: 'left' | 'center' | 'right' }).textAlign ?? 'center',
        textVerticalAlign: (e as { textVerticalAlign?: 'top' | 'center' | 'bottom' }).textVerticalAlign ?? 'center',
        stroke: (e as { stroke?: string }).stroke ?? '#dee0e3',
    ...
    case 'mindmap': {
      const factory = hasMindNodeFactory() ? getMindNodeFactory() : null;
      const defaultBranch = (factory?.defaultBranchStyle as string | undefined) ?? 'curve';
      return {
        ...base,
        type: 'mindmap',
        layout: (e as { layout?: string }).layout || 'right',
        root: factory
          ? factory.normalize((e as { root?: unknown }).root)
          : { id: `mn_${Date.now()}`, text: '', children: [] },
