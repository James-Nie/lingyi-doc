...
const FLOWCHART_PLACEMENT: Partial<Record<ShapeKind, Partial<ShapeElementDefaults>>> = {
  ...
  documentWavy: flowchartPlacementDefaults({ text: '文档', fill: '#f3e8ff', width: 120, height: 96, stroke: '#9333ea', strokeWidth: 2 }),
  parallelogram: flowchartPlacementDefaults({ text: '数据', fill: '#e8f5e9' }),
  cylinder: flowchartPlacementDefaults({ text: '数据库', fill: '#e8f5e9', stroke: '#52c41a', strokeWidth: 2 }),
  flowDataFlow: flowchartPlacementDefaults({ text: '数据流', fill: '#e8f5e9', stroke: '#52c41a', strokeWidth: 2 }),
  ...
  hexagon: flowchartPlacementDefaults({ text: '准备', fill: '#f5f5f5', stroke: '#9ca3af', strokeWidth: 2 }),
  flowOffPage: flowchartPlacementDefaults({ text: '跨页引用', fill: '#e3f2fd', width: 120, height: 88 }),
  flowQueue: flowchartPlacementDefaults({ text: '队列', fill: '#e8f5e9', width: 88, height: 88, stroke: '#52c41a', strokeWidth: 2 }),
};
...
const BUILTIN_SHAPE_DEFS: ShapeDefinition[] = [
  ...
  { kind: 'flowDataFlow', label: '数据流', defaults: () => baseDefaults(180, 80) },
  ...
  { kind: 'umlGeneralization', label: '继承', strokeOnly: true, defaults: smallDecorDefaults },
...
const DFD_BLUE = '#4d8df7';
const DFD_BLUE_FILL = '#dfe8ff';
const DFD_GREEN = '#5bd25f';
const DFD_GREEN_FILL = '#dff6df';
const DFD_STORE_STROKE = '#c9cdd4';

const DFD_PLACEMENT: Partial<Record<ShapeKind, Partial<ShapeElementDefaults>>> = {
  rect: {
    text: '外部实体',
    width: 150,
    height: 100,
...
export function resolvePlacementDefaults(
  ...
  if (categoryId === C.flowchart) return FLOWCHART_PLACEMENT[kind];
  if (categoryId === C.dfd) return DFD_PLACEMENT[kind];
  return undefined;
}
...
const BUILTIN_CATALOG: ShapeCatalogEntry[] = [
  ...
  // 泳道入口仍挂在图形库（图标预览）；放置时走 table 预设，不再创建 shape
  ...
  { kind: 'umlInterface', categoryId: C.classDiagram, order: 1 },
  { kind: 'umlClass2', categoryId: C.classDiagram, order: 2 },
  { kind: 'umlPackage', categoryId: C.classDiagram, order: 3, label: '类目' },
  ...
  { kind: 'dfdDataStore', categoryId: C.dfd, order: 1, label: '数据处理' },
  { kind: 'circle', categoryId: C.dfd, order: 2, label: '数据处理' },
  { kind: 'process', categoryId: C.dfd, order: 3, label: '数据处理' },
  { kind: 'dfdSubProcess', categoryId: C.dfd, order: 4, label: '数据存储' },
  { kind: 'dfdStoreOpenRight', categoryId: C.dfd, order: 5, label: '数据存储' },
  { kind: 'dfdStoreOpenLeft', categoryId: C.dfd, order: 6, label: '数据存储' },
