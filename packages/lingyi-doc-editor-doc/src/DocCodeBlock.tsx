...
import type { CodeBlock } from '@lingyi-doc/core-doc';
import { extractPlainText, getCaretOffset, setCaretOffset } from '@lingyi-doc/core-doc';
...
export const DocCodeBlock: React.FC<DocCodeBlockProps> = ({
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'Tab') {
      ...
      const next = `${text.slice(0, offset)}  ${text.slice(offset)}`;
      el.textContent = next;
      setCaretOffset(el, offset + 2);
      onChange({ ...block, text: next }, false);
    }
  };
